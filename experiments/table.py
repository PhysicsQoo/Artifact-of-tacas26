...
# path2 = "../benchmarks/MQTBench/Random_Pauli/"
...
for file in progress_bar:
    ...
    if tools_switch['feynver']:
        ...
        # circuit1_qc = circuit1.replace('.qasm','.qc')
        # circuit2_qc = circuit2.replace('.qasm', '.qc')
        # convert_qasm_to_qc(circuit1)
        # convert_qasm_to_qc(circuit2)
    ...
    '''pyzx part'''
    if tools_switch['pyzx']:
        import pyzx as zx
        def pyzx_verify_process(queue, circuit1, circuit2):
            try:
                c1 = zx.Circuit.load(circuit1)
                c2 = zx.Circuit.load(circuit2)
                res = c1.verify_equality(c2, up_to_swaps=False, up_to_global_phase=True)
                queue.put(res)
            except Exception as e:
                queue.put(str(e))

        progress_bar.set_description(f'pyzx ({file_name})')
        zx_result, zx_time = safe_verify(pyzx_verify_process, circuit1, circuit2, timeout=timeout)
        if zx_result == True:
            results["pyzx_result"] = 'equivalent*'
        elif zx_result == False:
            results["pyzx_result"] = 'not_equivalent'
        else:
            results["pyzx_result"] = zx_result
        results["pyzx_time"] = zx_time
